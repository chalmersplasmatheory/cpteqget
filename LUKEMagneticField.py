...
class LUKEMagneticField:
    ...
    def load(self, filename, path=''):
        ...
        with h5py.File(filename, 'r') as f:
            if (f[PATH].dtype == 'S1') or (str(f[PATH].dtype).startswith('|S')):
                self.id = f[PATH][:].tostring().decode('utf-8')
            elif f[PATH].dtype == 'object':
                if f[PATH].shape == ():
                    self.id = f[PATH][()].decode()
                elif type(f[PATH][:][0]) == str:
                    self.id = f[key][:][0]
                else:
                    self.id = f[PATH][:][0].decode()
            else:
                self.id = f[PATH][:]
